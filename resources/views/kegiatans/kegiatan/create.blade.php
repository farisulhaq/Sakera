    ...
    <div class="col">
      <div class="card">
        <div class="card-content">
          <div class="card-body">
            <form class="form form-vertical" action="{{ route('kegiatan.store') }}" method="post">
              @csrf
              <div class="form-body">
                <div class="row">
                  <div class="col-12">
                    <div class="form-group has-icon-left">
                      ...
                      <div class="position-relative">
                        <input name="kode" type="text" class="form-control" placeholder="Kode Kegiatan"
                          id="first-name-icon" value="{{ old('kode') }}" />
                    ...
                    <div class="form-group has-icon-left">
                      ...
                      <select class="choices form-select" name="kegiatan_role_id">
                        @foreach ($kegiatanRole as $role)
                          <option {{ old('kegiatan_role_id') == $role->id ? 'selected' : '' }}
                    ...
                    <div class="form-group has-icon-left">
                      ...
                      <div class="position-relative">
                        <input name="target" type="text" class="form-control" placeholder="Target"
                          id="first-name-icon" value="{{ old('target') }}" />
                        <div class="form-control-icon">
                    ...
                    <div class="form-group has-icon-left">
                      ...
                      <select class="choices form-select" name="satuan_id">
                        @foreach ($satuans as $satuan)
                          <option {{ old('satuan_id') == $satuan->id ? 'selected' : '' }} value="{{ $satuan->id }}">
                            {{ $satuan->name }}</option>
                        @endforeach
                      </select>
                      {{-- error --}}
                    ...
                    <div class="form-group has-icon-left">
                      ...
                      <select class="choices form-select" name="tahun">
                        @foreach ($years as $year)
                          <option {{ old('tahun') == $year ? 'selected' : '' }} value="{{ $year }}">
                            {{ $year }}</option>
                        @endforeach
                      </select>
                      {{-- error --}}
                    ...
                    <div class="form-group has-icon-left">
                      ...
                      <select class="choices form-select" name="role_id">
                        @foreach ($seksis as $seksi)
                          <option {{ old('role_id') == $seksi->id ? 'selected' : '' }} value="{{ $seksi->id }}">
                            {{ $seksi->name }}</option>
                        @endforeach
                      </select>
                      {{-- error --}}
